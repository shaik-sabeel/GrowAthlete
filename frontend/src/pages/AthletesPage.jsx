...
//         {data.length > 0 && (
//           <div className="ap-athlete-grid">
//             {data && data.slice(0, 6).map((athlete, index) => (
//               <motion.div
//                 key={athlete._id}
//                 className="ap-athlete-card"
//                 variants={athleteCardVariants}
...
import { motion } from 'framer-motion';
import { useInView } from 'react-intersection-observer';
import { FaSearch, FaAngleDown, FaSyncAlt } from 'react-icons/fa';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get('/auth/all-users');
        setData(res.data);
        setFilteredAthletes(res.data); // initially show all athletes
      } catch (err) {
        console.error('Error fetching profile data:', err);
        alert('Failed to fetch athletes data', err);
      }
  ...
  return (
    <>
      ...
      <section className="ap-hero-section">
        ...
        <div className="ap-hero-content">
          ...
          <motion.p
            ...
            transition={{ delay: 0.2, ...heroTitleVariants.visible.transition }}
          >
            Discover the rising stars of Indian sports who are making waves{' '}
            <br /> with their exceptional talent and dedication.
          </motion.p>
        </div>
      </section>
      ...
      <section className="ap-filter-section" ref={filterRef}>
        <div className="ap-filter-container container">
          <div className="ap-filter-input-group">
            <input
              type="text"
              placeholder="Filter Athletes by Name, Sport, or City"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="ap-filter-input"
            />
          ...
          <div className="ap-filter-actions">
            ...
            <Button
              variant="primary"
              onClick={() => {
                /* Search is real-time already */
              }}
            >
              <FaSearch /> Search
            </Button>
            <button
              className="ap-reset-filters-button"
      ...
      <section className="ap-filtered-results-section container">
        ...
        {filteredAthletes.length > 0 && (
          <div className="ap-athlete-grid">
            {filteredAthletes.slice(0, 6).map((athlete, index) => (
              <motion.div
                key={athlete._id}
                className="ap-athlete-card"
                variants={athleteCardVariants}
                ...
                transition={{
                  delay: index * 0.1,
                  ...athleteCardVariants.visible.transition,
                }}
                whileHover={{
                  y: -5,
      ...
      <section className="ap-discover-more-section container" ref={discoverRef}>
          ...
          {ATHLETE_CARDS_DATA.slice(0, 4).map((athlete, index) => (
            ...
            >
              ...
              <div className="ap-athlete-info">
                ...
                <p>{athlete.sport}</p>
                <small>{athlete.achievements}</small>
                <Button variant="link" link={`/athletes/${athlete.id}`}>
                  View Profile &rarr;
                </Button>
              </div>
            </motion.div>
        ...
        <div className="ap-discover-more-button-container">
          <Button variant="primary" link="/athletes-full-directory">
