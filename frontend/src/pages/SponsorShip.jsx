  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-black text-white p-4">
      <div className="relative text-center">
        {/* Animated "COMING" text */}
        <div className="text-6xl md:text-8xl font-bold tracking-wider leading-tight">
          {coming.split('').map((char, index) => (
            <span key={`coming-${index}`}
                  className={`inline-block opacity-0 ${animationClass}`}
                  style={{animationDelay: `${index * 0.1}s`}} // Adjusted delay for a faster build-up
            >
              {char}
            </span>
          ))}
        ...
        {/* Animated "SOON" text */}
        <div className="text-6xl md:text-8xl font-bold tracking-wider leading-tight mt-2 md:mt-4">
          {soon.split('').map((char, index) => (
            <span key={`soon-${index}`}
                  className={`inline-block opacity-0 ${animationClass}`}
                  style={{animationDelay: `${(coming.length + index) * 0.1}s`}} // Adjusted delay
            >
              {char}
            </span>
          ))}
        ...
        {/* Optional: Add the @effectualdesign text */}
        <p className="mt-8 text-xl font-light opacity-80 animate-fade-in delay-2000" style={{ animationDelay: '2s' }}>
            GROWATHLETE
        </p>
      </div>
    </div>
